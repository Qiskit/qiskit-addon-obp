...
- Effectiveness depends on how Clifford the circuit is
    - For high-magic circuits (i.e. very non-Clifford), the terms in the observable tend to have larger coefficients, which means one will generally incur a large amount of truncation error earlier during backpropagation, limiting the depth savings under a fixed error budget. For circuits that are near-Clifford, one can generally truncate terms more aggressively and backpropagate deeper into the circuit while staying under a fixed error budget.
